...
if __name__ == '__main__':
    ...
    for tup in sorted_hub_scores[:500]:
      if tup[1] > max_hub:
        max_hub = tup[1]
      if tup[1] < min_hub:
        min_hub = tup[1]
    ...
    std_auth = 0
    i = 0
    auths = np.ones(shape=(500))

    for tup in sorted_auth_scores[:500]:
      if tup[1] > max_auth:
        max_auth = tup[1]
      if tup[1] < min_auth:
        min_auth = tup[1]
    ...
    print('max = '+str(max_auth))
    print('min = '+str(min_auth))
    print('median = '+str(median_auth/2))
    print('std = '+str(np.std(auths)))
