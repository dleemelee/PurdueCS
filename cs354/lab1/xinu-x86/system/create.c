	...
	)
{
  ...
  //lab2var
	prptr->prextprio = priority; //changed to ext prio
  prptr->prrecent = priority;
  if(prptr->prrecent < 0) {
    prptr->prrecent = 0;
  }
  prptr->prbaseprio = 50;
  prptr->prquantum = QUANTUM+prptr->prrecent;
  prptr->prprio = prptr->prbaseprio + (2*prptr->prextprio) + prptr->prrecent;//lab2var done
