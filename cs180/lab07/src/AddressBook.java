...
public class AddressBook {
    ...
    public boolean addContact(Contact contact) {
        if( totalContacts == maxContacts ) {
            //Create new array with maxlength *2
            Contact[] temp = new Contact[maxContacts * 2];
            for( int i = maxContacts; i < temp.length; i++ ) {
                temp[i] = contacts[i-maxContacts];
            }
            contacts = temp;
            maxContacts*=2;
        }
        ...
        for( int i = 0; i < contacts.length; i++ ) {
            if( contacts[i] != null ) {
                if (contacts[i].getName().equals(contact.getName()) && contacts[i].getAddress() == null &&
                        contacts[i].getNumber() == 0){
            ...
            if( contacts[i] == null ) {
                ...
                if( contact.getNumber() != 0 ) {
                    if( contact.getAddress()!= null){
                        contacts[i] = new Contact(contact.getName(), contact.getNumber(), contact.getAddress());
                        totalContacts++;
                        return true;
                    }
                    contacts[i] = new Contact(contact.getName(), contact.getNumber());
                    totalContacts++;
                    return true;
                }
                contacts[i] = new Contact(contact.getName());
                totalContacts++;
                return true;
            }
        }
