/*
 * CS-252 Spring 2017
 * fliz.y: parser for the FLIZ interpreter
 */
...
%token DEFINE HALT OPENPAR CLOSEPAR STARTLIST ENDLIST
...
%type <const_val> const_expr const_expr_list
...
const_expr:
 STARTLIST const_expr_list ENDLIST
  { 
	$$ = $2;	
  }
|
...
| const_expr_list const_expr
  {
  	int i;
	struct VALUE * index =  $1 -> list.head;
	for(i = 0; i < $1 -> list.numVals-1; i++) {
		index = index->list.next;
	}
	index->list.next = $2;
    $1 -> list.tail -> list.next = $2;
	$1 -> list.tail = $2;
	$1 -> list.numVals++;
	$$ = $1;
  }
